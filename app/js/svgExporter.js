 ...
 * (UPDATED FOR TRUE LAYERED SVG EXPORT)
 */
...
class SVGExporter {
  ...
  /**
   * Generate combined SVG from all channels with true, editable layers for design software.
   */
  generateCombinedSVG(width, height, channels = ['cyan', 'magenta', 'yellow', 'black']) {
    ...
    // Add metadata
    svg += `<title>Combined CMYK Halftone Pattern (Layered)</title>`;
    svg += `<desc>Generated by Advanced Halftone Plotter Tool at ${new Date().toISOString()}. Each channel is in a separate group for easy editing in design software.</desc>`;

    // --- NEW: Generate a <style> block for clean, editable layers ---
    svg += `<defs>`;
    ...
    channels.forEach(channel => {
      ...
      if (channelSvg) {
        // --- UPDATED: Use class instead of inline style for the group ---
        svg += `<g id="${channel}-channel" class="${channel}-layer">`;
        svg += `<title>${channel.toUpperCase()} Channel</title>`;

        // Extract elements from channel SVG. They will inherit the fill from the group class.
        const elements = this.extractSVGElements(channelSvg);
        // --- UPDATED: No longer need to pass color to enhanceElements ---
  ...
  /**
   * Enhance elements with classes and optimizations.
   * (UPDATED: No longer adds inline fill/stroke)
   */
  enhanceElements(elements, channel) {
    let enhanced = elements;

    // --- REMOVED: The following lines are no longer needed as color is inherited from the parent group's class. ---
    // enhanced = enhanced.replace(/fill="[^"]*"/g, `fill="${color}"`);
    // enhanced = enhanced.replace(/stroke="currentColor"/g, `stroke="${color}"`);

    // Add channel-specific classes for advanced CSS styling if needed
